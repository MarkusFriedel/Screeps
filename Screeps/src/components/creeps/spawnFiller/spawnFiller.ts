...
class SpawnFiller extends MyCreep {

    public get memory(): SpawnFillerMemory { return this.creep.memory; }
        ...
        if (this.creep.ticksToLive <= 20) {
            ...
            if (this.creep.carry.energy == 0) {
                ...
                if (this.mainRoom.room.energyAvailable < this.mainRoom.room.energyCapacityAvailable) {
                    ...
                    let targets = this.creep.room.find<Spawn | Extension>(FIND_MY_STRUCTURES, { filter: (s: Spawn | Extension) => (s.structureType == STRUCTURE_SPAWN || s.structureType == STRUCTURE_EXTENSION) && s.energy < s.energyCapacity && !_.any(this.mainRoom.managers.spawnFillManager.creeps, c => c.name != this.creep.name && c.memory.targetStructureId && c.memory.targetStructureId == s.id) });
                    if (target == null)
