...
class Builder extends MyCreep<ConstructorMemory> {
        ...
        if (!this.pickUpEnergy(3)) {
            ...
            if (!this.memory.fillupContainerId) {
                if (this.creep.room.name == this.mainRoom.name)
                    ...
                    let possbibleContainers = _.map(_.filter(this.myRoom.mySources, s => (s.hasKeeper == false || !s.keeper.lair || s.keeper.lair.ticksToSpawn > 100 || s.keeper.creep && s.keeper.creep.hits <= 100) && s.container), s => s.container);
